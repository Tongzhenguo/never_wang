...
def train_test_gen( batch_size=32,maxlen=200,drop=0,train_num =32000,isTrain=True ):
    '''
    训练集（及验证集）批生成器
    :param batch_size: int,批次大小
    :param maxlen: int,最大填充序列，如果大于maxlen截断，小于则向后填充0
    ...
    if isTrain:
        while True:
            x_batch,y_batch = [],[]
            while( len(x_batch)<batch_size ):
                randi = random.randint(0,train_num-1)
                line = doc_list[randi]
                y = int(ys[randi])-1
                ids = [dictionary.token2id[token] for token in token_extract(line) if token in dictionary.token2id]
        ...
        for n in range(train_num,num_doc,batch_size):
            x_batch, y_batch = [], []
            i = n
            while ( i < i+batch_size):
                line = doc_list[n]
                y = int(ys[n]) - 1
                ids = [dictionary.token2id[token] for token in token_extract(line) if token in dictionary.token2id]
...
model_checkpoint = ModelCheckpoint('../model/cnn_eval.h5', save_best_only=False, save_weights_only=True, mode='min')
model.fit_generator(train_test_gen(batch_size,maxlen=maxlen,drop=0,train_num=32000,isTrain=True)
                    ,steps_per_epoch=int(40000/batch_size)+1
                    ,class_weight=class_weight
                    ,epochs=nb_epoch
                    ,callbacks=[early_stopping,model_checkpoint]
                    # ,validation_data=train_test_gen(batch_size,maxlen=maxlen,drop=0,train_num=32000,isTrain=False)
                    # ,validation_steps=1
                    )

### eval
clf = load_model('../model/cnn_eval.h5')
clf2 = load_model('../model/cnn.h5')
y_pred = []
y_pred2 = []
y_true = []
for x_batch,true_batch in train_test_gen(batch_size,maxlen=maxlen,drop=0,train_num=32000,isTrain=False):
    ...
    y_batch = clf2.predict(x_batch, batch_size=batch_size)
    for pred_prob in y_batch:  ## get label
        y_pred2.append(np.argmax(pred_prob) + 1)  # remapping to 1-8
print( metrics.f1_score(y_true=y_true,y_pred=y_pred,labels=[1,2,3,4,5,6,7,8],average='micro') )
print( metrics.f1_score(y_true=y_true,y_pred=y_pred2,labels=[1,2,3,4,5,6,7,8],average='micro') )
